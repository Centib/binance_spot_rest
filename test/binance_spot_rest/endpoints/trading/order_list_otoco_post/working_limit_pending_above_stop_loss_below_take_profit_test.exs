defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitPendingAboveStopLossBelowTakeProfitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitPendingAboveStopLossBelowTakeProfitQuery

  def full_valid_query do
    %WorkingLimitPendingAboveStopLossBelowTakeProfitQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(WorkingLimitPendingAboveStopLossBelowTakeProfitQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (optional):" do
    ...
    test "valid without optional fields" do
      assert {:ok, %WorkingLimitPendingAboveStopLossBelowTakeProfitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
               ~>> Map.drop(@optional)
               ~>> then(&struct(WorkingLimitPendingAboveStopLossBelowTakeProfitQuery, &1))
