defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitMakerPendingAboveLimitMakerBelowStopLossLimitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitMakerPendingAboveLimitMakerBelowStopLossLimitQuery

  def full_valid_query do
    %WorkingLimitMakerPendingAboveLimitMakerBelowStopLossLimitQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(
                   &struct(WorkingLimitMakerPendingAboveLimitMakerBelowStopLossLimitQuery, &1)
                 )
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  ...
  describe "validation (optional):" do
    ...
    test "valid without optional fields" do
      assert {:ok, %WorkingLimitMakerPendingAboveLimitMakerBelowStopLossLimitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
               ~>> Map.drop(@optional)
               ~>> then(
