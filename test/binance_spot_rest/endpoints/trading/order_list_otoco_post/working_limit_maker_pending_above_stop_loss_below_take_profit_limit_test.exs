defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitMakerPendingAboveStopLossBelowTakeProfitLimitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitMakerPendingAboveStopLossBelowTakeProfitLimitQuery

  def full_valid_query do
    %WorkingLimitMakerPendingAboveStopLossBelowTakeProfitLimitQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(
                   &struct(WorkingLimitMakerPendingAboveStopLossBelowTakeProfitLimitQuery, &1)
                 )
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  ...
  describe "validation (optional):" do
    ...
    test "valid without optional fields" do
      assert {:ok, %WorkingLimitMakerPendingAboveStopLossBelowTakeProfitLimitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
               ~>> Map.drop(@optional)
               ~>> then(
