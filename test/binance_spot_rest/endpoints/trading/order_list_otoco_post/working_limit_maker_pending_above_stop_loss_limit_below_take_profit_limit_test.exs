defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitMakerPendingAboveStopLossLimitBelowTakeProfitLimitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitMakerPendingAboveStopLossLimitBelowTakeProfitLimitQuery

  def full_valid_query do
    %WorkingLimitMakerPendingAboveStopLossLimitBelowTakeProfitLimitQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(
                   &struct(WorkingLimitMakerPendingAboveStopLossLimitBelowTakeProfitLimitQuery, &1)
                 )
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  ...
  describe "validation (optional):" do
    ...
    test "valid without optional fields" do
      assert {:ok, %WorkingLimitMakerPendingAboveStopLossLimitBelowTakeProfitLimitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
               ~>> Map.drop(@optional)
               ~>> then(
