defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitMakerPendingAboveTakeProfitLimitBelowStopLossTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtocoPost.WorkingLimitMakerPendingAboveTakeProfitLimitBelowStopLossQuery

  def full_valid_query do
    %WorkingLimitMakerPendingAboveTakeProfitLimitBelowStopLossQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(
                   &struct(WorkingLimitMakerPendingAboveTakeProfitLimitBelowStopLossQuery, &1)
                 )
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  ...
  describe "validation (optional):" do
    ...
    test "valid without optional fields" do
      assert {:ok, %WorkingLimitMakerPendingAboveTakeProfitLimitBelowStopLossQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
               ~>> Map.drop(@optional)
               ~>> then(
