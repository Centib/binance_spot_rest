defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitPendingStopLossLimitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitPendingStopLossLimitQuery

  def full_valid_query do
    %WorkingLimitPendingStopLossLimitQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(WorkingLimitPendingStopLossLimitQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (inclusive) (pending):" do
    ...
    test "valid with one of [:pendingStopPrice, :pendingTrailingDelta]" do
      assert {:ok, %WorkingLimitPendingStopLossLimitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
