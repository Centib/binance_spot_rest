defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitPendingTakeProfitLimitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitPendingTakeProfitLimitQuery

  def full_valid_query do
    %WorkingLimitPendingTakeProfitLimitQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(WorkingLimitPendingTakeProfitLimitQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (inclusive) (pending):" do
    ...
    test "valid with one of [:pendingStopPrice, :pendingTrailingDelta]" do
      assert {:ok, %WorkingLimitPendingTakeProfitLimitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
