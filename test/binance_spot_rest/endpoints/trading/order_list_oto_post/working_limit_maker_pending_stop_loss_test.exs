defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitMakerPendingStopLossTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitMakerPendingStopLossQuery

  def full_valid_query do
    %WorkingLimitMakerPendingStopLossQuery{
      ...
      pendingType: BinanceSpotRest.Enums.OrderType._STOP_LOSS(),
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(WorkingLimitMakerPendingStopLossQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (inclusive) (pending):" do
    ...
    test "valid with one of [:pendingStopPrice, :pendingTrailingDelta]" do
      assert {:ok, %WorkingLimitMakerPendingStopLossQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
