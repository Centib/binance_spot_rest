defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitMakerPendingTakeProfitLimitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitMakerPendingTakeProfitLimitQuery

  def full_valid_query do
    %WorkingLimitMakerPendingTakeProfitLimitQuery{
      ...
      pendingType: BinanceSpotRest.Enums.OrderType._TAKE_PROFIT_LIMIT(),
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(WorkingLimitMakerPendingTakeProfitLimitQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (inclusive) (pending):" do
    ...
    test "valid with one of [:pendingStopPrice, :pendingTrailingDelta]" do
      assert {:ok, %WorkingLimitMakerPendingTakeProfitLimitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
