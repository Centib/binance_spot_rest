defmodule BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitPendingStopLossTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOtoPost.WorkingLimitPendingStopLossQuery

  def full_valid_query do
    %WorkingLimitPendingStopLossQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(WorkingLimitPendingStopLossQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (inclusive) (pending):" do
    ...
    test "valid with one of [:pendingStopPrice, :pendingTrailingDelta]" do
      assert {:ok, %WorkingLimitPendingStopLossQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
