defmodule BinanceSpotRest.Endpoints.Trading.OrderListOcoPost.AboveTakeProfitLimitBelowStopLossLimitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOcoPost.AboveTakeProfitLimitBelowStopLossLimitQuery

  def full_valid_query do
    %AboveTakeProfitLimitBelowStopLossLimitQuery{
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(AboveTakeProfitLimitBelowStopLossLimitQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (inclusive) (above):" do
    ...
    test "valid with one of [:aboveStopPrice, :aboveTrailingDelta]" do
      assert {:ok, %AboveTakeProfitLimitBelowStopLossLimitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
