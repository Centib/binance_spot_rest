defmodule BinanceSpotRest.Endpoints.Trading.OrderListOcoPost.AboveTakeProfitBelowStopLossLimitTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOcoPost.AboveTakeProfitBelowStopLossLimitQuery

  def full_valid_query do
    %AboveTakeProfitBelowStopLossLimitQuery{
      ...
      aboveStopPrice: Decimal.new("20.0"),
  ...
  describe "validation (required):" do
    @required [:symbol, :side, :quantity, :belowTimeInForce, :belowPrice]
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(AboveTakeProfitBelowStopLossLimitQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (inclusive) (above):" do
    ...
    test "valid with one of [:aboveStopPrice, :aboveTrailingDelta]" do
      assert {:ok, %AboveTakeProfitBelowStopLossLimitQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
