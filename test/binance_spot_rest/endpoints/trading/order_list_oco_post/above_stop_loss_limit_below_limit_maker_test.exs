defmodule BinanceSpotRest.Endpoints.Trading.OrderListOcoPost.AboveStopLossLimitBelowLimitMakerTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Trading.OrderListOcoPost.AboveStopLossLimitBelowLimitMakerQuery

  def full_valid_query do
    %AboveStopLossLimitBelowLimitMakerQuery{
  ...
  describe "validation (required):" do
    @required [:symbol, :side, :quantity, :belowPrice, :aboveTimeInForce, :abovePrice]
    ...
    for field <- @required do
      test "invalid without #{field}" do
        assert {:error, %{field: unquote(field)}} =
                 ...
                 ~>> then(&struct(AboveStopLossLimitBelowLimitMakerQuery, &1))
                 ~>> BinanceSpotRest.Query.validate()
      end
    end
  end
  ...
  describe "validation (inclusive):" do
    ...
    test "valid with one of [:aboveStopPrice, :aboveTrailingDelta]" do
      assert {:ok, %AboveStopLossLimitBelowLimitMakerQuery{}} =
               full_valid_query()
               ~>> Map.from_struct()
