defmodule BinanceSpotRest.Endpoints.Account.RateLimitOrderTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Account.RateLimitOrder

  def full_valid_query do
    %RateLimitOrder.Query{
      recvWindow: 3000
    }
  end
  ...
  describe "validation (optional):" do
    ...
    test "valid without optional fields" do
      assert {:ok, %RateLimitOrder.Query{}} =
               full_valid_query()
               ~>> Map.from_struct()
               ~>> Map.drop(@optional)
               ~>> then(&struct(RateLimitOrder.Query, &1))
               ~>> BinanceSpotRest.Query.validate()
    end
  end
