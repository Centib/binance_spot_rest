defmodule BinanceSpotRest.Endpoints.Account.OpenOrdersTest do
  @moduledoc false
  ...
  alias BinanceSpotRest.Endpoints.Account.OpenOrders
  ...
  describe "validation (optional):" do
    ...
    test "valid without optional fields" do
      assert {:ok, %OpenOrders.Query{}} =
               full_valid_query()
               ~>> Map.from_struct()
               ~>> Map.drop(@optional)
               ~>> then(&struct(OpenOrders.Query, &1))
               ~>> BinanceSpotRest.Query.validate()
    end
  end
